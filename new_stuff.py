...
1801258597 : "Bounty",
2025709351 : "Vehicle",
2401704334 : "Warmind Emotes",
1345459588: "Warmind Quest Step", 
1107761855: "Emote Wheel" }
            ...
            if "displayProperties" in a and "hash" in a:
                ...
                if name == "":
                    if "itemTypeDisplayName" in a:
                        name = a["itemTypeDisplayName"]
                    elif "plugWhitelist" in a:
                        name = a["plugWhitelist"][-1]["categoryIdentifier"]
                ...
                if "inventory" in a:
                    if "bucketTypeHash" in a["inventory"]:
                        if a["inventory"]["bucketTypeHash"]>0:
                            if a["inventory"]["bucketTypeHash"] in bucket_types:
                                bucketTypeHash = bucket_types[a["inventory"]["bucketTypeHash"]]
                            else:
                                bucketTypeHash = "Unknown"
                                print("missing bucket_types: %s" % a["inventory"]["bucketTypeHash"])
                    if "tierTypeName" in a["inventory"]:
                        if len(a["inventory"]["tierTypeName"])>0:
                            tierTypeName = a["inventory"]["tierTypeName"]
                            
                if description == "":
                    if "progressDescription" in a:
                        description = a["progressDescription"]
...
def get_local_files():
    files = glob.glob('world_sql_content_*.content')
    
    files_dates = {}
    
    for file in files:
        current = creation_date(file)
        files_dates[current] = file
        
    sorted_keys = sorted(files_dates.keys(),reverse=True)
    if len(sorted_keys) >=2:
        return (files_dates[sorted_keys[0]],files_dates[sorted_keys[1]])
    else:
        print("You need at least 2 apis to compare")
        return False

newest = get_local_files()

old_items = db_read(newest[1])
new_items = db_read(newest[0])

#old_items = db_read("world_sql_content_7ed5ecc0b6c7188a689f25e3ceabc57b.content")
#new_items = db_read("world_sql_content_5b6870694dc15572dc92beb29a1693d2.content")
...
for item in old_items:
    if item in new_items:
        if old_items[item][0] != new_items[item][0] or old_items[item][1] != new_items[item][1]:
            ...
            changed[item].append("%s" % (new_items[item][6]))
        del new_items[item]
    else:
...
for item in new_items:
    ...
    if new_items[item][3]!="":
        ...
        print("   %s" % new_items[item][5])
    # put bucket type here if nothing
    print("  </td>")
    
    print("  <td>")
    print("   %s" % new_items[item][6])
    print("  </td>")
